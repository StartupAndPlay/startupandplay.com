 ...
 */
function wpuf_get_attachments( $post_id ) {
    $att_list = array();
...
function wpuf_show_meta_front( $content ) {
    ...
    if ( $show_attachment == 'on' ) {
        $attach = '';
        $attachments = wpuf_get_attachments( $post->ID );
...
function wpuf_feat_img_html( $attach_id ) {
    $image = wp_get_attachment_image_src( $attach_id, 'single-content-width' );
    ...
    $html .= sprintf( '<a class="wpuf-del-ft-image" href="#" data-id="%d"><span style="color: red;"><i class="icon-trash fontawesome"></i>%s</a></span>', $attach_id, __( 'Delete', 'wpuf' ) );
